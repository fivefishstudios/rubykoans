...
class AboutHashes < Neo::Koan
  ...
  def test_accessing_hashes_with_fetch
    ...
    assert_raise(KeyError) do   #KeyError raised if key does not exist
  ...
  def test_changing_hashes
    ...
    assert_equal expected, hash   # use variable for assert_equal

    # Bonus Question: Why was "expected" broken out into a variable
    # rather than used as a literal?
    #
    # Answer: Ruby thinks that { ... } is a block for assert_equal
  end
  ...
  def test_default_value
    ...
    assert_equal "dos", hash2[:two]   # takes on default value of "dos" as defined in .new
  end
  ...
  def test_default_value_is_the_same_object
    ...
    assert_equal true, hash[:one].object_id == hash[:two].object_id     # WHY????
  end
  ...
  def test_default_value_with_block
    ...
    assert_equal ["uno"], hash[:one]
    assert_equal ["dos"], hash[:two]
    assert_equal [], hash[:three]    # we have a default [] value
  end
end
